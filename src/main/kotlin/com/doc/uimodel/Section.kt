...
class Section(complexObject: CCOMPLEXOBJECT, orderInParent: Int) {
    ...
    init {
        ...
        header = termDefinitions["${archetypeRoot.archetypeId.value}-${archetypeRoot.nodeId}"]?.description
    ...
    fun getTerm(key: String, path: String): TermDefinition {
        return termDefinitions[key] ?: error("Term Definition not found for $key in element $path")
    }
}